        ...
        $(window).load(function() {
            ...
            function teseract(image, successCallback, errorCallback) {
            ...
            var demo_opt = function(){
                ...
                this.train_pattern = function() {
                    ...
                    templateImg.onload = function() {
                        ...
                        var sc0 = Math.min(max_pattern_size/template_img_u8.cols, max_pattern_size/template_img_u8.rows);
                        new_width = (template_img_u8.cols*sc0)|0;
                        new_height = (template_img_u8.rows*sc0)|0;

                        jsfeat.imgproc.resample(template_img_u8, lev0_img, new_width, new_height);
            ...
            function id_detection(ctx) {
                // get the projected pattern corners
                var shape_pts = tCorners(homo3x3.data, pattern_preview.cols*2, pattern_preview.rows*2);

                //Check frame dimentions
                var dX1 = shape_pts[1].x - shape_pts[0].x;
                var dY1 = shape_pts[2].y - shape_pts[1].y;
                var h1 = Math.sqrt(dX1 * dX1 + dY1 * dY1);

                var dX2 = shape_pts[2].x - shape_pts[3].x;
                var dY2 = shape_pts[3].y - shape_pts[0].y;
                var h2 = Math.sqrt(dX2 * dX2 + dY2 * dY2);

                if(h1 - h2 < 5 && h1 - h2 > -5 ) {
                    var tempContext = tempCanvas.getContext('2d');

                    var sectionWidth = ((0.68 - 0.08) * Math.min(dX1, dX2)) + 12;
                    var sectionHeight = ((1 - 0.69) * Math.min(dY1, dY2)) + 12;
                    var sectionOriginX = (shape_pts[0].x + Math.min(dX1, dX2) * 0.08) - 8;
                    var sectionOriginY = (shape_pts[0].y + Math.min(dY1, dY2) * 0.7) - 8;


                    tempCanvas.width = sectionWidth;
                    tempCanvas.height = sectionHeight;

                    tempContext.drawImage(ctx.canvas, sectionOriginX, sectionOriginY, tempCanvas.width, tempCanvas.height, 0, 0, tempCanvas.width, tempCanvas.height);
                    ...
                    teseract(image, ocrResponse, function() {alert('ajax error');});
                }
